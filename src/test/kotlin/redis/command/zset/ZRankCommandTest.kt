...
class ZRankCommandTest {
    ...
    private lateinit var command: ZRankCommand
    ...
    fun `존재하는 멤버의 순위를 반환한다`() {
        ...
        val args = listOf(
            RESPValue.BulkString("ZRANK".toByteArray()),
    ...
    @Test
    fun `String 타입의 키에 zrank하면 WrongTypeException을 던진다`() {
    ...
    @Test
    fun `만료된 키에서 zrank하면 null을 반환한다`() {
        zsetOps.zadd("myzset", listOf(1.0 to "member1".toByteArray()))
