...
class RPopCommandTest {
    ...
    fun setUp() {
        ...
        command = RPopCommand(listOps)
    }

    @Test
    fun `리스트에서 마지막 요소를 꺼낸다`() {
        listOps.rpush("mylist", listOf("a".toByteArray(), "b".toByteArray(), "c".toByteArray()))

        val args = listOf(
            RESPValue.BulkString("RPOP".toByteArray()),
    ...
    @Test
    fun `String 타입의 키에 rpop하면 WrongTypeException을 던진다`() {
