...
class RPushCommandTest {
    ...
    private lateinit var command: RPushCommand
    ...
    fun setUp() {
        ...
        command = RPushCommand(listOps)
    }

    @Test
    fun `새 키에 값을 추가하면 리스트 길이를 반환한다`() {
        val args = listOf(
            RESPValue.BulkString("RPUSH".toByteArray()),
    ...
    fun `기존 리스트에 값을 추가하면 전체 길이를 반환한다`() {
        listOps.rpush("mylist", listOf("first".toByteArray()))
    ...
    @Test
    fun `RPUSH는 오른쪽에 순서대로 추가한다`() {
        listOps.rpush("mylist", listOf("a".toByteArray(), "b".toByteArray(), "c".toByteArray()))

        val result = listOps.lrange("mylist", 0, -1)

        assertThat(result.map { String(it) }).containsExactly("a", "b", "c")
    }
    ...
    @Test
    fun `String 타입의 키에 rpush하면 WrongTypeException을 던진다`() {
    ...
    @Test
    fun `만료된 키에 rpush하면 새 리스트를 생성한다`() {
        listOps.rpush("mylist", listOf("old".toByteArray()))
