...
class MSetCommandTest {
    ...
    fun `여러 키-값을 한번에 저장한다`() {
        ...
        assertThat(stringOps.get("key1")?.toString(Charsets.UTF_8)).isEqualTo("value1")
        assertThat(stringOps.get("key2")?.toString(Charsets.UTF_8)).isEqualTo("value2")
    }

    @Test
    fun `기존 값을 덮어쓴다`() {
        stringOps.set("key1", "old".toByteArray())
        val args = listOf(
            RESPValue.BulkString("MSET".toByteArray()),
            RESPValue.BulkString("key1".toByteArray()),
        ...
        assertThat(stringOps.get("key1")?.toString(Charsets.UTF_8)).isEqualTo("new")
    }
